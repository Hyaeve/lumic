...
| Twitter | 通过 twitterapi.io API Key 接入；填写推特用户名后可同步该账号点赞动态 |
...
推特平台使用 [twitterapi.io](https://twitterapi.io/) 接口，不再使用 X 官方 OAuth 访问令牌。请在设置 → 平台凭证中填写 `API Key` 和推特用户名（不含 `@`）。Lumic 会先请求用户资料验证凭证，再通过 `twitterapi.io` 拉取该账号的点赞动态；API Key 只保存在服务端加密配置中，不会回显给浏览器。
...
- 搜索同时匹配正文、作者名称和标签。
- 全部动态默认随机展示；点击排序按钮可切换最新/最早。向下浏览后，右下角的随机刷新按钮可以恢复随机排序并换一批内容。平台、作者与标签页默认按时间排序。
- 动态按游标分批加载，只渲染当前视口附近的卡片；返回已浏览页面时复用分页缓存。
- 列表正文默认最多显示十行，可展开全文；瀑布流模式以第一张图片作为封面，桌面端自适应 3–5 列，手机端固定 2 列。
- 手机卡片与桌面瀑布流显示月/日，桌面列表显示月/日和时分；桌面悬停时间可查看完整年月日时分。
- 桌面瀑布流详情采用左图右文布局，图片区支持滚轮与箭头切图，正文独立滚动。作者、标签和收藏页顶部从对应图库选取原图背景，每 15 秒渐变切换；作者与标签页统计当前筛选范围，收藏页仅显示收藏数量。
- 在这些页面顶部，桌面端向上滚动可将右侧内容区展开为清晰背景幻灯片，向下滚动收起；手机端下拉展开、上滑收起，也可使用关闭按钮或系统返回退出。
...
动态接口支持 `source`、`liked`、`author`、`tag`、`q`、`feedId`（订阅来源）与 `id`（单条动态）筛选。`order` 支持 `newest`、`oldest` 和 `random`，接口默认仍为 `newest`，兼容现有客户端。

随机浏览使用 `order=random&seed=<本轮随机种子>`，后续页保持相同的筛选条件与 `seed`，并携带 `nextCursor`。更换 `seed` 后从第一页开始即可换一批；不要把旧游标用于新种子。响应的 `total` 是当前筛选范围的总数，作者、标签、订阅来源和收藏筛选的首页还提供最多 12 张 `headerMedia` 原图背景及 `scopeStats`（当前筛选范围的总数、今日数量和收藏数量），不影响 `stats` 中的全局统计。
...
作者、标签及收藏页使用原图背景，普通顶栏每 15 秒轮换，大图模式每 7 秒轮换。
页面位于顶部时，桌面向上滚动逐步展开大图，向下滚动逐步收回；手机下拉展开、
上滑收回。背景尺寸、清晰度和遮罩跟随手势进度变化，超过约六成进度后松手完成
切换，不足则平滑回弹。手机大图可单击退出，拖拽、长按和多指触控不会触发单击退出；
大图也可通过系统返回或 Escape 退出。

提供 PWA manifest、主屏幕图标及 iOS 安全区域适配。通过 HTTPS 访问后，可在
iOS Safari 的分享菜单选择“添加到主屏幕”，以独立窗口打开，仍按屏幕宽度自动
切换移动端布局。Service Worker 不缓存账号接口、动态、私人图片或应用资源，
离线仅显示连接提示；不提供离线归档浏览。普通局域网 HTTP 仍可正常浏览，
但不启用 Service Worker（localhost 除外）。
