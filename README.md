...
- 桌面瀑布流详情采用左图右文布局，图片区支持滚轮与箭头切图，正文独立滚动。作者与标签页顶部从对应图库选取背景，每 12 秒渐变切换。
...
随机浏览使用 `order=random&seed=<本轮随机种子>`，后续页保持相同的筛选条件与 `seed`，并携带 `nextCursor`。更换 `seed` 后从第一页开始即可换一批；不要把旧游标用于新种子。响应的 `total` 是当前筛选范围的总数，作者、标签和订阅来源的首页还提供最多 12 张 `headerMedia` 预览背景。
