...
- 点击列表或详情中的平台图标可编辑、删除动态；删除需二次确认。编辑支持正文及图片添加、替换、移除和排序，保存后的本地修改不会被平台同步覆盖。图片限 JPEG/PNG/GIF/WebP，单张 20MB、请求总量 64MB、最多 50 张；移除图片仅解除当前动态引用，不立即删除原文件。
...
- 图片查看器及手机详情页加载原图时保留预览图，显示圆环加载状态；原图解码完成后在固定位置淡入，不刷新图片容器。失败可重试，圆环不显示估算百分比。
- 手机卡片与桌面瀑布流显示月/日，桌面列表显示月/日和时分；桌面悬停时间可查看完整年月日时分。
- 手机瀑布流长按卡片直接进入多选，不选中文本。向下浏览时悬浮按钮隐去，向上回看时浮现；iOS 顶部按钮为 50px，位于顶部安全区下方 20px。
- 桌面瀑布流详情采用左图右文布局，图片区支持滚轮与箭头切图，正文独立滚动。作者、标签和收藏页顶部从对应图库选取原图背景，每 15 秒渐变切换；作者与标签页统计当前筛选范围，收藏页仅显示收藏数量。
- 在这些页面顶部，桌面端悬浮背景区域后点击括弧展开幻灯片，点击底部括弧或按 Esc 收起；手机端下拉展开、上滑收起，也可使用系统返回退出。
...
随机浏览使用 `order=random&seed=<本轮随机种子>`，后续页保持相同的筛选条件与 `seed`，并携带 `nextCursor`。更换 `seed` 后从第一页开始即可换一批；不要把旧游标用于新种子。响应的 `total` 是当前筛选范围的总数，作者、标签、订阅来源和收藏筛选的首页还提供最多 12 张 `headerMedia` 原图背景及 `scopeStats`（当前筛选范围的总数、今日数量和收藏数量），不影响 `stats` 中的全局统计。轮播通过 `GET /api/v1/gallery` 按作者、标签、订阅来源或收藏范围持续补充随机原图，每次从该范围的全部图片中重新抽样，返回最多 12 张 `media` 和对应的 `postIds`，不受列表分页限制。
...
作者、标签及收藏页使用原图背景，普通顶栏每 15 秒轮换。桌面悬浮顶栏背景区域
（仅头像及标题文字不触发）显示内容区中线上的呼吸箭头，离开顶栏 1 秒后缓慢淡出。
点击展开大图，箭头沿同一中线移动到底部并平滑转向，
再次点击收回。桌面滚轮不切换模式，大图每 9 秒轮换，左右边缘悬浮按钮可手动换图，
Escape 可退出。

手机在页面顶部下拉展开、上拉收回，尺寸与遮罩跟随手势进度变化，未达阈值平滑回弹。
其阻尼曲线、距离阈值及松手吸附/回弹节奏与手机瀑布流详情页上下翻动态一致。
手机不显示箭头或左右按钮，横向滑动换图，大图每 10 秒轮换；单击或系统返回也可退出。
桌面双击、手机单指双击可直接退出并打开图片所属动态；
背景图片与动态通过接口关联，即使该动态尚未加载到列表也可定位，不额外叠加返回记录。
轮播持续从当前作者、标签或收藏的全部原图中随机补充候选，不只循环首页的背景样本。

提供 PWA manifest、主屏幕图标及 iOS 安全区域适配。通过 HTTPS 访问后，可在
iOS Safari 的分享菜单选择“添加到主屏幕”，以独立窗口打开，仍按屏幕宽度自动
切换移动端布局。主页模式采用沉浸式状态栏，页面背景延伸至顶部；轻磨砂仅覆盖
真实的顶部安全区，在悬浮按钮上方结束，不使用整块不透明黑色遮罩。
Service Worker 不缓存账号接口、动态、私人图片或应用资源，
